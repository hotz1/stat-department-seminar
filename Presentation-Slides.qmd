...
::: {.fragment fragment-index=1}
- The black-box optimization problem which we want to solve is:
$$x^{\star} = \text{argmax}_{x \in \mathcal{X}}\tilde{f}(x)$$
:::
...
:::: {.fragment}
![](img/GP-example-plots/GP-diff-lenscales.png){.absolute top=200 left=0 width=40%}
::::

:::: {.fragment}
![](img/GP-example-plots/GP-diff-noises.png){.absolute top=0 left=700 width=40%}
::::

:::: {.fragment}
![](img/GP-example-plots/GP-diff-outputscales.png){.absolute bottom=0 left=700 width=40%}
::::
...
- Our simulation study utilized the following settings:
  * Two benchmark datasets: **mopta08 (124 dimensions)** and **svm (388 dimensions)**
  * Two acquisition functions: Upper Confidence Bound and **Expected Improvement**
  * Four distance-based kernel functions: Matern(1/2), Matern(3/2), Matern(5/2), and **RBF Kernel**
  * Four models: **MLE (no priors)**, **Gamma(3,6) prior**, **$D$-scaled prior**, and **$D$-scaled prior with outputscale**
...
![](img/gelato-spoon/mopta-1000/comparison-by-model-filtered.png){.absolute top=0 right=0 width=40%}

![](img/gelato-spoon/mopta-1000/comparison-by-kernel-type-filtered.png){.absolute bottom=0 right=0 width=40%}

![](img/gelato-spoon/mopta-1000/comparison-by-acq-function-filtered.png){.absolute top=100 left=0 width=60%}
:::
...
![](img/gelato-spoon/svm-1000/comparison-by-model-filtered.png){.absolute top=0 right=0 width=40%}

![](img/gelato-spoon/svm-1000/comparison-by-kernel-type-filtered.png){.absolute bottom=0 right=0 width=40%}

![](img/gelato-spoon/svm-1000/comparison-by-acq-function-filtered.png){.absolute top=100 left=0 width=60%}
:::
